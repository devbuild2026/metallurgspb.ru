...
const cityTexts = [
  ...
  'За металлопрокатом {in} обращаются и частные застройщики, и крупные подрядчики. Держим на складе ходовые позиции, поэтому отгружаем без ожидания.',
  'Нужен металл {in}? Подберём марку и размер, рассчитаем объём и привезём {by} в удобное время. {distanceNote}.',
  'Со склада в Петербурге регулярно отгружаем заказы {by}. {name} — направление, по которому машины уходят почти ежедневно.',
  'Комплектуем объекты {in} под ключ: от арматуры и труб до листа и сетки. Один поставщик — одна машина, минимум согласований.',
  'Цены на прокат {in} держим без наценок перекупщиков — работаем с заводами напрямую. Срок доставки — {deliveryTime}.',
  'Привозим металл {by} собственным транспортом, без посредников и перегрузок. {distanceNote}, поэтому укладываемся в {deliveryTime}.',
  'Снабжаем стройки и мастерские {in} круглый год. Складской запас в Санкт-Петербурге позволяет не ставить заказы в очередь.',
];
...
const heroSubVariations = [
  ...
  'Арматура, трубы, лист и профнастил — всё со склада в Питере',
  'Привезём {by} собственными машинами — {deliveryTime}',
  '500+ позиций в наличии. Доставка {by} без задержек',
  'Металл от завода — без наценок и лишних звеньев',
];
